...
3. Validate MCP payload against `schemas/mcp.schema.json` when `mcp_servers`/spec payload is present.
...
- `docs/features/openapi/*` references still showing top-level `spec_file/base_url/filters`.
