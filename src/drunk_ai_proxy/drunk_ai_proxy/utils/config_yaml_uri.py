...
class ConfigYamlUriBuilder:
    """Class-first URI derivation helpers for remote resources."""
    ...
    def build_skill_resource_uris(
        ...
        if resource_name:
            skill_name = ConfigYamlUriBuilder.normalize_resource_name(resource_name)
        else:
    ...
    def build_agent_resource_uri(url: str, resource_name: str | None = None) -> str:
        ...
        if resource_name:
            normalized_name = ConfigYamlUriBuilder.normalize_resource_name(resource_name)
        ...
        if not filename.endswith(".agent.md"):
            filename = (
                f"{filename.rsplit('.', 1)[0]}.agent.md"
                if "." in filename
                else f"{filename}.agent.md"
            )
    ...
    def build_prompt_resource_uri(url: str, resource_name: str | None = None) -> str:
        ...
        if resource_name:
            return f"prompt://{ConfigYamlUriBuilder.normalize_resource_name(resource_name)}"
