...
class TestMcpProxyProviderCreateProxy:
    ...
    @patch('app.GlobalAuthProvider')
    @patch('src.proxies.mcp_proxy_provider.FastMCP')
    def test_create_proxy_returns_cached_instance(self, mock_fastmcp_cls, mock_auth_provider):
    ...
    def test_create_proxy_with_root_path_uses_root_mcp(self, mock_fastmcp_cls, mock_auth_provider):
    ...
    @patch('src.proxies.mcp_proxy_provider.SERVER_NAME', 'test-server')
    @patch('src.proxies.mcp_proxy_provider.SERVER_VERSION', '2.0.0')
    def test_create_proxy_with_non_root_path_creates_new_mcp(self, mock_fastmcp_cls, mock_auth_provider):
    ...
    def test_create_proxy_calls_create_proxy_method(self, mock_create_proxy_fn, mock_fastmcp_cls, mock_auth_provider):
        ...
        mock_create_proxy_fn.assert_called_once_with(mock_config.spec_data, name=mock_config.path)
...
class TestMcpProxyProviderCreateMcpProxiesConfigs:
    ...
    @patch('src.proxies.mcp_proxy_provider.SERVER_NAME', 'test-server')
    @patch('src.proxies.mcp_proxy_provider.SERVER_VERSION', '1.0.0')
    def test_create_mcp_proxies_configs_creates_root_mcp(self, mock_fastmcp_cls, mock_auth_provider):
    ...
    @patch('app.GlobalAuthProvider')
    @patch('src.proxies.mcp_proxy_provider.FastMCP')
    def test_create_mcp_proxies_configs_skips_none_spec_data(self, mock_fastmcp_cls, mock_auth_provider):
        ...
        result = McpProxyProvider.create_mcp_proxies_configs([mock_config1, mock_config2])
    ...
    @patch('app.GlobalAuthProvider')
    @patch('src.proxies.mcp_proxy_provider.FastMCP')
    def test_create_mcp_proxies_configs_processes_multiple_configs(self, mock_fastmcp_cls, mock_auth_provider):
        """Test create_mcp_proxies_configs processes multiple valid configs."""
        mock_config1 = Mock(spec=SpecConfig)
        mock_config1.path = "/api1"
        mock_config1.spec_data = {"mcpServers": {"test1": {"url": "http://example1.com"}}}
        mock_config1.skill_dir = None
        
        mock_config2 = Mock(spec=SpecConfig)
        mock_config2.path = "/api2"
        mock_config2.spec_data = {"mcpServers": {"test2": {"url": "http://example2.com"}}}
        mock_config2.skill_dir = None
